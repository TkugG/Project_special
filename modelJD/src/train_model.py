...
def train_and_save_model():
    print("=" * 60)
    print("🚀 Starting Calibrated Machine Learning Model Training (Pure Skill Architecture)")
    print("=" * 60)
    ...
    if not CSV_PATH.exists():
        CSV_PATH = DATA_DIR / "kaggle_jobs.csv"
    if not CSV_PATH.exists():
        CSV_PATH = DATA_DIR / "IT_Job_Roles_Skills.csv"
    if not CSV_PATH.exists():
    ...
    clean_resps = df_jobs.get("Responsibilities", df_jobs.get("Job Description", "")).apply(clean_text_pipeline)
    clean_keywords = df_jobs.get("Keywords", "").apply(clean_text_pipeline)
    ...
    # TF-IDF Vectorizer (Section 8.2 Text Feature Extraction)
    ...
    job_tfidf_matrix = vectorizer.fit_transform(df_jobs["combined_features"])
    ...
    for i in range(num_jobs):
        ...
        for _ in range(3):
            ...
            other_title = str(df_jobs.iloc[j]["Title"])
            if other_skills:
    ...
    # 4. Train Scaled Logistic Regression Pipeline (C=0.1 for smooth probabilistic calibration)
    print("⏳ Fitting StandardScaler + LogisticRegression(C=0.1) pipeline...")
    pipeline = Pipeline([
    ...
    print("\nClassification Report:\n", classification_report(y_test, y_pred, target_names=["Mismatched (0)", "Matched (1)"]))

    # Inspect Weights & Learned Intercept
    ...
    print("=" * 60)
    print("📐 LEARNED LOGISTIC REGRESSION WEIGHTS (Standardized Space):")
    print("=" * 60)
    print(f"  • Intercept (beta_0):               {intercept:+.4f}")
    print(f"  • tfidf_sim (beta_1):               {coefs[0]:+.4f}")
    print(f"  • overlap_ratio (beta_2):           {coefs[1]:+.4f}")
    print(f"  • exact_count (beta_3):             {coefs[2]:+.4f}")
    print("=" * 60 + "\n")
