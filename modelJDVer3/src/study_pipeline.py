...
def step3_extract_features(df_jobs: pd.DataFrame, user_skills: list, target_role: str = ""):
    ...
    for idx in range(len(df_jobs)):
        ...
        # คำนวณค่า 3 ฟีเจอร์ (พร้อม Zero-division guard)
        x1_tfidf_sim = float(np.clip(similarity_scores[idx], 0.0, 1.0))
        x3_exact_cnt = float(len(matched))
        x2_coverage_ratio = float(x3_exact_cnt / len(job_skills)) if len(job_skills) > 0 else 0.0

        X_features.append([x1_tfidf_sim, x2_coverage_ratio, x3_exact_cnt])
        candidate_profiles.append({
            ...
            "x2_coverage": x2_coverage_ratio,
            "x3_exact": x3_exact_cnt
        })

    # แสดงตารางตัวอย่าง 3 แถวแรก
    print("📊 ตารางตัวอย่างค่าฟีเจอร์ [X1, X2, X3] สำหรับ 3 ตำแหน่งงานแรก:")
    print("-" * 80)
    print(f"{'No.':<4} | {'Job Title':<35} | {'X1 (Semantic)':<13} | {'X2 (Coverage)':<13} | {'X3 (Count)'}")
    print("-" * 80)
    for i in range(3):
        p = candidate_profiles[i]
        print(f"{i+1:<4} | {p['title'][:35]:<35} | {p['x1_semantic']:<13.4f} | {p['x2_coverage']:<13.4f} | {int(p['x3_exact'])}")
    print("-" * 80)
...
def step4_predict_and_rank(X_matrix: np.ndarray, candidate_profiles: list, user_pref: str = "", model_path = None, top_k: int = 5):
    """
    ขั้นตอนที่ 4: นำค่า X1, X2, X3 เข้าโมเดล Logistic Regression เพื่อคำนวณ Match Score (0-100%)
    
    📐 ที่มาของสมการโมเดลและการสอบเทียบ (Model Rigor & Calibration):
    ------------------------------------------------------------------------------------
    1. โมเดล Logistic Regression Sigmoid Function:
       P(y=1 | X) = σ(z) = 1 / (1 + e^-(beta_0 + beta_1*X1 + beta_2*X2 + beta_3*X3))
       โดยฟีเจอร์ผ่านการทำ Standard Scaling (Mean=0, Std=1) ก่อนเข้าสมการ Sigmoid
       
    2. สูตรคะแนนสอบเทียบความน่าเชื่อถือ (Calibrated Composite Match Score):
       Match Score = 50% * P(y=1) + 35% * (Skill Coverage Ratio) + 15% * (TF-IDF Semantic)
       ช่วยแก้ปัญหา Overconfidence ในกรณีที่มีทักษะตรงเพียง 1 ตัว ไม่ให้คะแนนโดดสูงเกินจริง
    ...
    for i, profile in enumerate(candidate_profiles):
        ...
        coverage = float(profile["x2_coverage"])
        semantic = float(profile["x1_semantic"])
        
        # คำนวณคะแนนรวมตามสูตร Calibrated Composite Score
        composite_score = (0.50 * ml_prob + 0.35 * coverage + 0.15 * semantic) * 100.0
    ...
    for rank, job in enumerate(top_results, 1):
        ...
        print(f"  มิติการคำนวณ     : Semantic={int(job['x1_semantic']*100)}% | Coverage={int(job['x2_coverage']*100)}% | Exact={int(job['x3_exact'])} ทักษะ")
