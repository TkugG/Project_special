...
window.JobComponents = {
    ...
    renderResponsibilities(resps) {
        ...
        const boilerplate = [
            /^are you an/i, /^we are looking/i, /^about us/i, /^who we are/i,
            /^join us/i, /^is a leading/i, /^established in/i, /^what you will do/i
        ];
        ...
        const cleanItems = items.filter(item => {
            ...
            if (item.length > 130) {
                return item.substring(0, 127).replace(/[\s,;.-]+$/, '') + '...';
            }
            return item;
        });

        if (cleanItems.length === 0) {
            return `<li class="text-slate-500 font-medium italic">ปฏิบัติหน้าที่และพัฒนาโซลูชันตามมาตรฐานสายงานที่ได้รับมอบหมาย</li>`;
        }

        return cleanItems.slice(0, 4).map(item => `
